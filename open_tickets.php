...
?>
			...
			<div id="content">
				<div class="title_wrap"><div class="title">Open Tickets</div></div>
				<?php
					include_once 'inc/db.php'; // Includes DB Connection
					$opentickets = "SELECT ticket_id,title,status_name,priority_name,category_name,username
					...
					$opentickets_result = mysqli_query($db,$opentickets);
						
					if(mysqli_num_rows($opentickets_result) > 0) {
						...
						while ($row = mysqli_fetch_assoc($opentickets_result)) {
						...
						echo "<div style='width:300px;margin:0 auto;font-weight:bold;font-style:italic;padding-top:15px;'>Currently there are no open tickets</div>";
					}
				mysqli_free_result($opentickets_result);
				mysqli_close($db);
				?>
			</div>
			<div class="clear"></div>
